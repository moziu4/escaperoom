...
import { WINDOW_SIZE, ITEM_SIZE } from "../utils/constants"

export function* watchMoveRequest(action) {
  const { key } = action
  const { snakePos, foodPos, isGameOver, currentScore } = yield select(
    (state) => state.snakeServiceReducer
  )
  ...
  if (!isGameOver) {
    ...
    if (
      snakePos.length > 1 &&
      newPos.x === snakePos[1].x &&
      newPos.y === snakePos[1].y
    ) {
      return yield put(actions.moveFailure())
    }
